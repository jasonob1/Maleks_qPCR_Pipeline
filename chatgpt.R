...
server <- function(input, output) {
  ...
  output$tableO <- renderTable({
    ...
    if (!is.null(inFile) && !is.null(input$data) && inFile$datapath != "") {
      if (file.exists(inFile$datapath)) {
        if (inFile$name == "metadata.xlsx") {
        ...
        # Handle the case where the metadata.xlsx file doesn't exist
        "Metadata file not found."
      }
    } else {
      # Handle the case where required inputs are missing
