...
server <- function(input, output) {
  ...
  output$mergedTable<-DT::renderDataTable({
    ...
    # Initialize factors
    factors <- reactiveVal(c("Sample"))
    
    # Render additional factor dropdowns
    output$factorDropdowns <- renderUI({
      lapply(factors(), function(factor) {
        selectInput(factor, label = factor, choices = c("Option1", "Option2"))
      })
    })
    
    # Output the selected factors
    output$result <- renderText({
      selected_factors <- lapply(factors(), function(factor) {
        input[[factor]]
      })
      paste("Selected Factors:", paste(unlist(selected_factors), collapse = ", "))
    })
